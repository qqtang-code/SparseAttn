...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config: PawQwen3Config, context_window_toggle: Optional[int] = 1024):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        **kwargs,
            ...
            if new_len > past_kv.size(1):
                ...
                past_kv = torch.cat(
                    [
                        past_kv,
                        torch.empty(
                            hidden_states.size(0), 256, 2, kv.size(3), kv.size(4),
        ...
        dummy_z_sum = torch.tensor(0.0, device=hidden_states.device, dtype=hidden_states.dtype)
        dummy_entropy = torch.tensor(0.0, device=hidden_states.device, dtype=hidden_states.dtype)
        dummy_pooled = None
        dummy_z_contrast = torch.tensor(0.0, device=hidden_states.device, dtype=hidden_states.dtype) # 或者 None，视外层 loss 计算而定
        attn_weights = None

        return dummy_z_sum, dummy_entropy, dummy_pooled, dummy_z_contrast, attn_output, attn_weights, present_key_value
...
class Qwen3DecoderLayer(nn.Module):
    def __init__(self, config: PawQwen3Config, context_window_toggle: Optional[int] = 4096):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Qwen3Attention(config=config, context_window_toggle=context_window_toggle)
    ...
    def set_threshold_for_deterministic(self, val): pass
    def get_masks(self): return None
    def reset_masks(self, val=4.0): pass
    def fill_masks_with_value(self, val): pass
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs,
        ...
        if not return_dict:
            return tuple(v for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, model_sparsity, target_sparsity_out, z_loss] if v is not None)
